        ...
        SUBROUTINE VUEL(nblock,rhs,amass,dtimeStable,svars,nsvars, &
            ...
            double precision :: pNNquad(iGPquad,iNODE-1)      ! shape function matrix (interpolation matrix)
            ...
            double precision :: xi1quad,xi2quad,xi3quad         ! natural coordinates
	    ...
	    double precision :: NODES(iCORD)           ! physical coordinates
            ! integer
            integer :: kblock,ip,nn,ni,nj,i,ipquad,Filesize
            ! Allocatable arrays
	    ...
	    integer, DIMENSION(:), ALLOCATABLE :: BackEle            
			...
			if (ANY(jElem(kblock).eq.FrontEle)) then
        	        	    pGPCORDquad(1,:) = (/ 1/sqrt(3), 1/sqrt(3)/)
	                	    pGPCORDquad(2,:) = (/ -1/sqrt(3), 1/sqrt(3) /)
	                	    pGPCORDquad(3,:) = (/ 1/sqrt(3), -1/sqrt(3) /)
	                	    pGPCORDquad(4,:) = (/ -1/sqrt(3), -1/sqrt(3) /)
